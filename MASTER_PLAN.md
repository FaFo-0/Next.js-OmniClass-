...
| `/admin/people` | Students/Instructors tabs: pause/resume, teacher assignment, VacancyEditor (titled with the teacher's name), invites. Instructor rows show timezone + whether a meeting room is set. |
| `/admin/sessions` (+`/deleted`) | Org-wide sessions (with teacher + student columns) + soft-delete restore. |
| `/admin/library` (+`[id]`) | Add markdown materials, publish toggle, ✨ Prepare (batch vocabulary pre-resolve, opt-in), reading preview. ⚠️ no PDF/audio file upload UI. |
...
| `/admin/billing` | Packs tab (region-grouped catalog CA/Gulf, editor, seed) + pack-aware grant flow + remove-lessons + **Records** = the `pointTransactions` ledger (who granted what, why, balance after). |
| `/admin/settings` | Branding + feature toggles (Achievements/Library actually gate the student sidebar; the other three are labelled "not enforced yet"), AI prompt manager, achievements, scheduling knobs. ⚠️ several dead buttons (§5). |
...
- **Both deployments were wiped clean 2026-07-30** (see change log) — teacher availability went with it, so **repaint availability on the teacher calendar** (prod included) before booking works again.
...
- Still open: **no teacher detail page** (Instructors names aren't clickable — timezone/meeting room/student count are inline on the row, but a teacher's own page doesn't exist). Sessions "View" routes into teacher paths. Library: no PDF/audio upload. Settings: prompt Edit/Test dead, achievements Edit dead, logo upload dead, no invite-link copy/rotate UI, `/sign-up?invite=` wrapper unbuilt. Gamification / Live Quiz Generation / Payments toggles save but nothing reads them yet.
