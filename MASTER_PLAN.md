...
> **NEXT UP (2026-07-23, FaFo re-prioritized):** **Build now, test at the very end; phone/mobile pass is dead last.** P2 retention machinery is **dropped** — superseded by POLICY.md (packs + 60-day first-use expiry + pause + attention list, already built §see change log 07-19→07-23). Remaining build work: ① Phase Z teacher tabs polish (Students detail page Z.T.STU-1/2, Library markdown Z.T.LIB-2, Dashboard revisit). ② §14.6 P2 polish (hover cards, copy-week, admin multi-teacher view, touch tap-select C-10). ③ Billing "points→lessons" copy sweep. ④ POLICY [PROPOSED] items awaiting build/decision (consent checkbox §8, expiry warnings 14/3d §2, homework 24h publish obligation §10). **Then** full testing pass on desktop, **then** phone last.
