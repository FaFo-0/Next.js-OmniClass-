...
> **Status:** Phase G (UI port) — **COMPLETE (2026-05-08).** Student, teacher, and admin portals match the Omnica-new-UI prototype: dark purple sidebar gradient, gold active items, cream canvas, purple shadows + scrollbar, `.tbl/.card/.tab/.pill/.lesson-row` component classes, calendar consistent across student + teacher. Backend gaps (SRS due-cards, reviewLogs, quizAttempts, studySessions) wired. Confirmed by Mustafa.
