...
| 2026-05-07 | **[DeepSeek V4 Pro]** Phase G — Student portal rebuilt 1:1 from Omnica-new-UI. All 8 pages ported with exact JSX structure, inline styles, icons, and prototypes CSS classes — only data layer swapped from `MOCK` to Convex `useQuery`/`useAuth`. Pages: `/student` (welcome + streak + Next Up hero + Study Due + stat cards + recent lessons), `/student/lessons` (search + tabs + lesson rows), `/student/lessons/[id]` (summary expandable, vocab grid with TTS, flashcard flip preview, interactive quiz with scoring), `/student/study` (deck overview + SRS card flip with 3D CSS animation + 4-button rating + session complete celebration), `/student/vocabulary` (searchable table with TTS, filter chips), `/student/achievements` (stat cards + locked/unlocked grid with progress bars), `/student/profile` (avatar + stats + subscription progress bar), `/student/calendar` (day/week/month toggle, toolbar). `/student/library` ported with full reading view (word click → popover, font size controls, CEFR filters). `convex/lessonContent.ts`: `listAllVocab` query added for vocab aggregation. |
